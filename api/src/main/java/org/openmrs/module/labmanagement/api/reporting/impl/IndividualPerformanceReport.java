...
import org.openmrs.module.labmanagement.api.dto.SummarizedTestReportItem;
...
public class IndividualPerformanceReport extends ReportGenerator {
    ...
    public void execute(BatchJob batchJob, Function<BatchJob, Boolean> shouldStopExecution) {
        ...
        try {
            ...
            List<SummarizedTestReportItem> data = labManagementService.getIndividualPerformanceReport(startDate, endDate, diagnosticLocationId, testerUserId);
            if (shouldStopExecution.apply(batchJob)) {
                return;
            }
    ...
    protected void writeRow(CSVWriter csvWriter, SummarizedTestReportItem row, Map<Integer, List<TestConfigDTO>> testConfig) {
        ...
        writeLineToCsv(csvWriter, formatName(row.getTesterFamilyName(), row.getTesterMiddleName(), row.getTesterGivenName()),
                row.getTestName(), group,  row.getTestsCompleted() == null ? "0" : Long.toString(row.getTestsCompleted()));
    }

    protected void writeHeaders(CSVWriter csvWriter) {
        writeLineToCsv(csvWriter, "User", "Test", "Group", "Count");
    }
}
