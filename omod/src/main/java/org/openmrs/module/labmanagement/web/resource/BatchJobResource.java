...
@Resource(name = RestConstants.VERSION_1 + "/" + ModuleConstants.MODULE_ID + "/batch-job", supportedClass = BatchJobDTO.class, supportedOpenmrsVersions = {"1.9.*", "1.10.*", "1.11.*", "1.12.*", "2.*"})
public class BatchJobResource extends ResourceBase<BatchJobDTO> {
    ...
    public BatchJobDTO getByUniqueId(String uniqueId) {
        ...
        Result<BatchJobDTO> jobs = getLabManagementService().findBatchJobs(batchJobSearchFilter);
    ...
    protected void delete(BatchJobDTO delegate, String reason, RequestContext context) throws ResponseException {
        ...
        for (String uuid : batchJobsToDelete){
            getLabManagementService().cancelBatchJob(uuid, reason);
        }
    }
    ...
    protected PageableResult doSearch(RequestContext context) {
        ...
        filter.setBatchJobTypes( Arrays.asList(BatchJobType.Report, BatchJobType.Migration));

        LabManagementService stockManagementService = getLabManagementService();
        ...
        Result<BatchJobDTO> result = getLabManagementService().findBatchJobs(filter);
    ...
    public BatchJobDTO save(BatchJobDTO delegate) {
        try {
            return getLabManagementService().saveBatchJob(delegate);
        } catch (Exception e) {
    ...
    @PropertySetter("parameters")
    public void setParameters(BatchJobDTO instance, Map<String, ?> items) {
        instance.setParametersMap(items);
    }
